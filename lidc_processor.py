...
def load_scan(path):
    ...
    slices = [pydicom.dcmread(os.path.join(path, s), force=True) 
...
def get_pixels_hu(slices):
    ...
    for slice_number, slice_data in enumerate(slices):
        if slice_data.RescaleSlope != 1:
            image[slice_number] = slice_data.RescaleSlope * image[slice_number].astype(np.float64)
...
def raw_processing(slice_pixels, window_center=20, window_width=400, **kwargs):
    ...
    return ((windowed_image - min_value) / (max_value - min_value) * 255).astype(np.uint8) / 255.0
...
def convert_dicom(input_dir, output_dir, method="segmented", window_center=20, window_width=400):
    ...
    try:
        ...
        if method == "segmented":
            ...
            count = 0
            for i, slice_pixels in enumerate(tqdm(patient_pixels)):
                processed = raw_processing(slice_pixels, window_center=window_center, window_width=window_width)
                output_path = os.path.join(output_dir, f"slice_{i+1:03d}.png")
                os.makedirs(output_dir, exist_ok=True)
                plt.imsave(output_path, processed, cmap=plt.cm.gray)
                count += 1
...
def main():
    parser = argparse.ArgumentParser(description='Process LIDC-IDRI dataset and convert DICOM to PNG')
    parser.add_argument('--input', required=True, help='Base path to LIDC-IDRI dataset')
    parser.add_argument('--output', required=True, help='Base path for output PNG files')
    parser.add_argument('--method', default="segmented", choices=["basic", "raw", "segmented"],
                       help='Conversion method: basic, raw, or segmented')
    parser.add_argument('--window-center', type=int, default=20, help='Window center for raw conversion')
    parser.add_argument('--window-width', type=int, default=400, help='Window width for raw conversion')
